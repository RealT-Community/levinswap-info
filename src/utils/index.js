...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://app.levinswap.realt.community/` +
      "#/" +
      (remove ? `remove` : `add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://app.levinswap.realt.community/swap?inputCurrency=${token0Address}`
  } else {
    return `https://app.levinswap.realt.community/swap?inputCurrency=${token0Address === '0xe91d153e0b41518a2ce8dd3d7944fa863463a97d' ? 'ETH' : token0Address
...
export const urls = {
  showTransaction: (tx) => `https://gnosisscan.io/tx/${tx}/`,
  showAddress: (address) => `https://gnosisscan.io/address/${address}/`,
  showToken: (address) => `https://gnosisscan.io/token/${address}/`,
  showBlock: (block) => `https://gnosisscan.io/block/${block}/`,
}
